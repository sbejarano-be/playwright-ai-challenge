...
| Reportes           | HTML de Playwright + Allure 3 (`allure-playwright` y la CLI `allure` de npm) | Playwright admite varios reporters a la vez, así que se cumplen las dos menciones del enunciado. Allure 3 no necesita Java.                                                                                                                                            |
...
- **Pruebas del propio framework:** el proyecto `framework` (21 pruebas, sin red, con una IA simulada) verifica las reglas de A, B y C: validación de propuestas, umbrales, reintentos, respaldos, repetición de datos y privacidad del contexto enviado a la IA.
